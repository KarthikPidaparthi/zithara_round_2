...
function HomeData() {
  ...
  const delRecord=useCallback(async sno=>{
    try {
      const delData=await fetch(`http://localhost:5000/customer/${sno}`,{
        method:"DELETE"
      });
      setRecords(records.filter(record=>record.sno!==sno));
    } catch (err) {
                ...
                {search.toLowerCase()===''
                    ?visibleRecord.map((record) => (
                      <tr key={record.sno}>
                        ...
                        <td>
                          <button onClick={()=>{delRecord(record.sno)}}>Delete</button></td>
                          <td> <EditData record={record}/>  </td>
                      </tr>
                    ))
                : records.filter((item)=>
                  ...
                  <tr key={record.sno}>
                    ...
                    <td>{formatDate(record.created_at)}</td>
                    <td>{formatTime(record.created_at)}</td>
                    <button onClick={()=>{delRecord(record.sno)}}>delete</button>
                    <td> <EditData record={record}/>  </td>
                  </tr>
                ))}
              </tbody>
